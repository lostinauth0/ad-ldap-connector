...
function getProfileMapper() {
  if (config.get('PROFILE_MAPPER')) {
    return eval(config.get('PROFILE_MAPPER'));
  } else if (config.get('PROFILE_MAPPER_FILE')) {
    return require(config.get('PROFILE_MAPPER_FILE'));
  } else {
    return require('./profileMapper');
  }
}
...
Users.prototype.enrichProfile = function (profile, noGroups, callback) {
  ...
  async.parallel([
    ...
    const profileMapper = getProfileMapper();
    if (!profileMapper) {
      callback(null, profile);
    } else if (profileMapper.length === 1) {
