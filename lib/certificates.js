...
class Certificates {
  ...
  async initialize(options = {}) {
    ...
    this.#cache.cert = pems.cert;
    this.#cache.key = pems.private;
  ...
  async #writeToFile(contents, filePath) {
    ...
    // Setup permissions for file so that only admins can read/write it.
    if (this.#process.platform !== 'win32') {
      await this.#exec(`chmod 600 ${filePath}`);
    } else {
